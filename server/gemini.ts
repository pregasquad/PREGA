...
`;
  ...
  const prompt = `أنتِ وسال — بنت مغربية حقيقية تشتغل في استقبال صالون ${ctx.name} وتردي على واتساب ديال الصالون.
...
` : ""}${(() => {
  ...
  return `━━━ شخصية وسال — امزجي هاد الصفات في كل رد ━━━\n${blocks.join("\n")}`;
})()}

━━━ مين أنتِ ━━━
• اسمك وسال، موظفة حقيقية في ${ctx.name}
• مرأة مغربية — ذكية، طبيعية في كلامها
• لو سألوكِ "واش نتِ بوت؟" أو "واش أنتِ روبوت؟" → ردي بعفوية تامة مثل: "آه لا والله أنا وسال 😄" أو "هههه لا غير أنا" أو "والله مبوت والو، أنا وسال 😊"
...
• بعض الردود في تاريخ المحادثة مكتوب قبلها "[رد المدير]:" — هاذ يعني صاحبة الصالون أو المدير ردات على العميلة مباشرة بنفسها (مش وسال)
...
export async function learnFromConversation(
  ...
  const conversationText = history
    .map((t) => `${t.role === "user" ? "العميلة" : "وسال"}: ${t.text}`)
    .join("\n");
...
استخرج المعلومات التالية وأجب بـ JSON فقط بدون أي تفسير أو نص خارج الـ JSON:
{
  ...
  "botErrors": [
    {
      "wrongInfo": "المعلومة الخاطئة التي قالها البوت (وسال) — نص موجز",
